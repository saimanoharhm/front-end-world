...
const whereAmI = function (lat, lng) {
  fetch(`https://geocode.xyz/${lat},${lng}?geoit=json`)
    .then(function (response) {
      if (!response.ok)
        throw new Error(
          `You are reloading faster (${response.status}), Do it a bit slower :D`
        );
      console.log(response);
      return response.json();
    })
...
const whereAmI = async function () {
  try {
    ...
    console.log(data);
    renderCountry(data[0]);
    return `You are in ${dataGeo.principalSubdivision}, ${dataGeo.countryName}`;
  } catch (err) {
    console.error(`${err} 💥`);
